...
class CustomTargetTransformer(BaseEstimator,TransformerMixin):
    """ computes the logarithmic value of the column passed.

    Examples
    --------
    if 'y' is a target variable .The way to transform and inverse transform 'y' will be as follows:
    
    >>> tranformer=CustomTargetTransformer()
    >>> transformed_y=transformer.fit_transform(y)
    >>> original_y=transformer.inverse_transform(transformed_y)
