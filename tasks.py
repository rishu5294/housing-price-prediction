...
@task(name="checkcomplexityscore")
def rad_func(c):
    try:
        import radon

        print("radon module found,checking complexity scores for each module")
    except:
        print("Radon module not found,Installing the radon module")
        lib = "radon"
        c.run(f"pip install {lib}")
    check_setup_prerequisites(c)
    path = os.path.join(os.getcwd(), "src")
    os.chdir(path)
    c.run(f"radon cc {path}")


_create_task_collection("test", rad_func)
